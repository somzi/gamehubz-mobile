...
function PlayerCardPoster({ name, avatarUrl, stats, width }: {
    ...
    // 88dp box captured at ~3x pixel ratio → ask Cloudinary for ~264px so the PNG stays sharp.
    const optimizedAvatar = avatarUrl ? getOptimizedCloudinaryUrl(avatarUrl, 264) : '';
    const sideLabel = { fontSize: 9, fontWeight: '700' as const, letterSpacing: 1.2, color: '#64748B', marginTop: 3 };

    return (
        <View style={{ width, backgroundColor: '#06070D', overflow: 'hidden', paddingHorizontal: 20, paddingTop: 26, paddingBottom: 20 }}>
            ...
            <View style={{
                ...
                <View style={{ paddingHorizontal: 22, paddingBottom: 24, marginTop: -44 }}>
                    {/* Avatar in a dark ring, overlapping the banner */}
                    <View style={{ alignSelf: 'center', width: 88, height: 88, borderRadius: 26, padding: 4, backgroundColor: '#0B111D' }}>
                        {optimizedAvatar ? (
                            <Image
                                source={{ uri: optimizedAvatar }}
                                style={{ width: '100%', height: '100%', borderRadius: 22, backgroundColor: '#1A1D2E' }}
                                contentFit="cover"
                                cachePolicy="memory-disk"
                            />
                        ) : (
                            <LinearGradient colors={['#2DD4ED', '#3B82F6']} style={{ flex: 1, borderRadius: 22, alignItems: 'center', justifyContent: 'center' }}>
                                <Text style={{ fontSize: 36, fontWeight: '800', color: '#06121C' }}>{initial}</Text>
                            </LinearGradient>
                        )}
                    </View>
                    ...
                    <Text
                        ...
                        style={{ marginTop: 12, fontSize: 30, fontWeight: '800', color: '#F8FAFC', textAlign: 'center', letterSpacing: -0.5 }}
                    >
                        {name}
                    </Text>
                    <View style={{ alignItems: 'center', marginTop: 8 }}>
                        <View style={{
                            ...
                            paddingVertical: 4,
                            borderRadius: 99,
                            backgroundColor: 'rgba(45,212,237,0.12)',
                            borderWidth: 1,
                            borderColor: 'rgba(45,212,237,0.3)',
                        }}>
                            <Text style={{ fontSize: 10, fontWeight: '800', letterSpacing: 2.2, color: '#2DD4ED' }}>PLAYER</Text>
                        </View>
                    </View>
                    ...
                    <View style={{ marginTop: 24, flexDirection: 'row', alignItems: 'center' }}>
                        <View style={{ flex: 1, alignItems: 'center' }}>
                            <Text style={{ fontSize: 26, fontWeight: '800', color: '#F8FAFC', lineHeight: 28 }}>{stats.matches}</Text>
                            <Text style={sideLabel}>MATCHES</Text>
                        </View>
                        <View style={{ width: DONUT_SIZE, height: DONUT_SIZE }}>
                            ...
                            <View style={[StyleSheet.absoluteFill, { alignItems: 'center', justifyContent: 'center' }]}>
                                <Text style={{ fontSize: 28, fontWeight: '800', color: '#F8FAFC', lineHeight: 30 }}>{rate}%</Text>
                                <Text style={{ fontSize: 9, fontWeight: '700', letterSpacing: 1.4, color: '#64748B', marginTop: 2 }}>WIN RATE</Text>
                            </View>
                        </View>
                        <View style={{ flex: 1, alignItems: 'center' }}>
                            <Text style={{ fontSize: 26, fontWeight: '800', color: '#FBBF24', lineHeight: 28 }}>{stats.trophies}</Text>
                            <Text style={sideLabel}>TROPHIES</Text>
                        </View>
                    </View>
            ...
            <View style={{ marginTop: 14, alignItems: 'center' }}>
                <Text style={{ fontSize: 11, fontWeight: '800', letterSpacing: 4 }}>
...
export function SharePlayerCardModal({ visible, onClose, playerId, name, avatarUrl, stats }: SharePlayerCardModalProps) {
    ...
    const posterWidth = Math.max(300, Math.min(340, windowWidth - 48));
    ...
    return (
        <Modal visible={visible} transparent animationType="fade" onRequestClose={onClose}>
            <Pressable className="flex-1 bg-black/70" onPress={onClose}>
                <ScrollView
                    contentContainerStyle={{ flexGrow: 1, alignItems: 'center', justifyContent: 'center', paddingVertical: 28, paddingHorizontal: 24 }}
